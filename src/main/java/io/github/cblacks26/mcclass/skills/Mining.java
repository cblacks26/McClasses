...
public class Mining extends Skill implements Listener{
	...
	public Mining(String name, String scoreboardName) {
	...
	public void onBlockBreak(BlockBreakEvent event){
		...
		if(getXP(block) > 0.0){
			switch(item.getType()){
				case DIAMOND_PICKAXE: case GOLD_PICKAXE: case IRON_PICKAXE: case STONE_PICKAXE: case WOOD_PICKAXE:
			...
			case DIAMOND_PICKAXE: case GOLD_PICKAXE: case IRON_PICKAXE: case STONE_PICKAXE: case WOOD_PICKAXE:
				if(event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)){
					...
					if(!(user.getUserSkillClass().isBound()))
						event.getPlayer().sendMessage("InstaMine Off");
					else
						event.getPlayer().sendMessage("InstaMine On");
				}
				break;
			default:
				break;

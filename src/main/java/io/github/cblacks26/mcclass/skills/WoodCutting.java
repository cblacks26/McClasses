...
public class WoodCutting extends Skill implements Listener{
	...
	public WoodCutting(String name, String scoreboardName) {
	...
	public void onBlockBreak(BlockBreakEvent event){
		...
		if(getXP(block) > 0.0){
			switch(item.getType()){
				case DIAMOND_AXE: case GOLD_AXE: case IRON_AXE: case STONE_AXE: case WOOD_AXE:
			...
			case DIAMOND_AXE: case GOLD_AXE: case IRON_AXE: case STONE_AXE: case WOOD_AXE:
				if(event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)){
					...
					if(!(user.getUserSkillClass().isBound()))
						event.getPlayer().sendMessage("InstaCut Off");
					else
						event.getPlayer().sendMessage("InstaCut On");
				}
				break;
			default:
				break;
	...
	public void loadSettings(FileConfiguration config) {
		ConfigurationSection section = config.getConfigurationSection("Skills.WoodCutting");
		if(section.getKeys(false)!=null){
			for(String s:section.getKeys(false)){
				if(Material.valueOf(s.toUpperCase()) != null){
					Material type = Material.valueOf(s.toUpperCase());
					Double val = config.getDouble("Skills.WoodCutting."+s);
					blocks.put(type, val);
				}
			}
		}
